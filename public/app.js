// ---- Admin login (simple) ----
const ADMIN_PASSWORD = '3551'; // keep in sync with server ADMIN_PASS

function adminIsLogged() {
  return sessionStorage.getItem('adminOk') === '1';
}

function promptAdminLogin() {
  const p = prompt('Enter admin password to enable add/edit/delete:');
  if (p === ADMIN_PASSWORD) {
    sessionStorage.setItem('adminOk', '1');
    alert('Admin access granted (temporary).');
    return true;
  }
...
// helper to add admin header if logged in
function fetchWithAdmin(url, options = {}) {
  options.headers = options.headers || {};
  if (adminIsLogged()) options.headers['x-admin-pass'] = ADMIN_PASSWORD;
  return fetch(url, options);
}
/* public/app.js — polling-enabled noticeboard frontend */
const apiRoot = 'https://vsb-noticeboard.onrender.com/api/notices';
