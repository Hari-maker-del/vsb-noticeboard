...
let me=JSON.parse(localStorage.getItem('vsb_user')||'null');
const cookie=name=>document.cookie.split('; ').find(x=>x.startsWith(name+'='))?.slice(name.length+1)||'';
const api=async(u,o={})=>{o={...o,credentials:'same-origin'};const method=String(o.method||'GET').toUpperCase();o.headers={...(o.headers||{})};if(o.body&&!(o.body instanceof FormData))o.headers['Content-Type']='application/json';if(!['GET','HEAD','OPTIONS'].includes(method)){const csrf=cookie('vsb_csrf');if(csrf)o.headers['X-CSRF-Token']=decodeURIComponent(csrf)}const r=await fetch(u,o);let d={};try{d=await r.json()}catch{}if(r.status===401&&me&&u!=='/api/auth/login'){me=null;localStorage.removeItem('vsb_user')}if(!r.ok)throw Error(d.error||'Request failed');return d};
...
function authUI(){
  const b=$('admin');if(!b)return;
  const name=me?E(me.name):'Sign in';
  const role=me?(me.role==='ADMIN'?'Administrator':me.role==='FACULTY'?'Faculty':'Student'):'Account access';
  b.innerHTML='<div class="avatar">'+(me?E((me.name||'V')[0].toUpperCase()):'VS')+'</div><div class="profile-copy"><b>'+name+'</b><span>'+role+'</span></div>';
  b.onclick=()=>me?(me.role==='ADMIN'?adminDashboard():me.role==='FACULTY'?workspace():profile()):login();
}
function profile(){if(!me)return login();let p=$('proProfile')||(()=>{const d=document.createElement('div');d.id='proProfile';d.className='fixed inset-0 z-[85] bg-slate-950/80 grid place-items-center p-4';document.body.append(d);return d})();p.innerHTML='<div class="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-sm w-full max-h-[90vh] overflow-auto"><div class="flex justify-between"><div><p class="text-xs uppercase tracking-widest text-blue-600 font-bold">Account</p><h2 class="text-2xl font-extrabold">'+E(me.name)+'</h2><p class="text-sm text-slate-500 mt-1">'+E(me.role)+' • '+E(me.email)+'</p></div><button id="closeProfile" class="text-slate-400">✕</button></div><form id="profileEditForm" class="space-y-2 mt-5"><div class="text-xs font-bold uppercase tracking-wider text-slate-500">Profile</div><input id="profileName" class="w-full border p-3 rounded-xl text-sm" value="'+E(me.name||'')+'" placeholder="Full name"><input id="profilePhone" class="w-full border p-3 rounded-xl text-sm" value="'+E(me.phone||'')+'" placeholder="Phone"><div class="grid grid-cols-2 gap-2"><input id="profileClass" class="w-full border p-3 rounded-xl text-sm" value="'+E(me.class_code||'')+'" placeholder="Class"><input id="profileRoll" class="w-full border p-3 rounded-xl text-sm" value="'+E(me.roll_no||'')+'" placeholder="Roll"></div><input id="profileSemester" class="w-full border p-3 rounded-xl text-sm" value="'+E(me.semester||'')+'" placeholder="Semester"><button class="w-full border border-blue-200 text-blue-700 p-3 rounded-xl font-semibold">Save profile</button><p id="profileMsg" class="text-xs min-h-4"></p></form><form id="changePasswordForm" class="mt-5 pt-5 border-t space-y-2"><div class="text-xs font-bold uppercase tracking-wider text-slate-500">Security</div><input id="currentPassword" type="password" required minlength="1" class="w-full border p-3 rounded-xl text-sm" placeholder="Current password"><input id="newPassword" type="password" required minlength="10" class="w-full border p-3 rounded-xl text-sm" placeholder="New password (10+ characters)"><button class="w-full bg-blue-600 text-white p-3 rounded-xl font-semibold">Change password</button><p id="passwordMsg" class="text-xs min-h-4"></p></form><div class="mt-5 pt-5 border-t"><div class="text-xs font-bold uppercase tracking-wider text-slate-500">Sessions</div><p class="text-[11px] text-slate-500 mt-1">View active sessions and sign out other devices.</p><div id="sessionList" class="mt-3 space-y-2"></div><button id="revokeOtherSessions" class="w-full mt-3 border border-amber-200 text-amber-700 p-3 rounded-xl font-semibold">Sign out other devices</button></div><button id="signOut" class="w-full mt-5 border border-red-200 text-red-600 p-3 rounded-xl font-semibold">Sign out</button></div>';p.classList.remove('hidden');
$('closeProfile').onclick=()=>p.remove();
const renderSessions=async()=>{try{const d=await api('/api/auth/sessions');$('sessionList').innerHTML=d.items.map(x=>'<div class="p-3 rounded-xl bg-slate-50 border"><div class="flex items-center justify-between gap-2"><span class="text-xs font-bold">'+(x.current?'Current session':'Active session')+'</span><span class="text-[10px] text-slate-400">'+new Date(x.created_at).toLocaleString()+'</span></div><div class="text-[10px] text-slate-500 mt-1 truncate">'+E(x.user_agent||'Unknown device')+' • '+E(x.ip_address||'Unknown IP')+'</div>'+(x.current?'':'<button class="text-[10px] text-red-600 mt-2" data-revoke-session="'+E(x.id)+'">Sign out this device</button>')+'</div>').join('')||'<p class="text-xs text-slate-500">No active sessions.</p>';document.querySelectorAll('[data-revoke-session]').forEach(b=>b.onclick=async()=>{try{await api('/api/auth/sessions/'+encodeURIComponent(b.dataset.revokeSession),{method:'DELETE'});msg('Session revoked');renderSessions()}catch(err){msg(err.message)}})}catch(err){$('sessionList').innerHTML='<p class="text-xs text-red-600">'+E(err.message)+'</p>'}};
$('profileEditForm').onsubmit=async e=>{e.preventDefault();try{const d=await api('/api/profile',{method:'PUT',body:JSON.stringify({name:$('profileName').value,phone:$('profilePhone').value,class_code:$('profileClass').value,roll_no:$('profileRoll').value,semester:$('profileSemester').value})});me=d.user;localStorage.setItem('vsb_user',JSON.stringify(me));authUI();msg('Profile updated');$('profileMsg').className='text-xs text-emerald-600';$('profileMsg').textContent='Profile saved.';if($('class')&&me.class_code)$('class').value=me.class_code}catch(err){$('profileMsg').className='text-xs text-red-600';$('profileMsg').textContent=err.message}};
$('changePasswordForm').onsubmit=async e=>{e.preventDefault();$('passwordMsg').textContent='';try{const d=await api('/api/profile/password',{method:'PUT',body:JSON.stringify({current_password:$('currentPassword').value,new_password:$('newPassword').value})});e.target.reset();$('passwordMsg').className='text-xs text-emerald-600';$('passwordMsg').textContent='Password changed. Sign in again to continue.';msg('Password updated');me=null;localStorage.removeItem('vsb_user');p.remove();authUI();login()}catch(err){$('passwordMsg').className='text-xs text-red-600';$('passwordMsg').textContent=err.message}};
$('revokeOtherSessions').onclick=async()=>{try{const d=await api('/api/auth/sessions/revoke-others',{method:'POST'});msg((d.revoked||0)+' other session(s) signed out');renderSessions()}catch(err){msg(err.message)}};
$('signOut').onclick=async()=>{try{await api('/api/auth/logout',{method:'POST'})}catch{}me=null;localStorage.removeItem('vsb_user');p.remove();authUI();msg('Signed out')};
renderSessions()}
...
const uploadFile=async file=>{const fd=new FormData();fd.append('file',file);const d=await api('/api/uploads',{method:'POST',body:fd});return d.url};
const adminState={};
...
async function adminLoad(section){
  ...
  try{
    ...
    if(section==='students'){
      ...
      $('adminContent').innerHTML='<div class="grid xl:grid-cols-[360px_1fr] gap-4"><form id="studentCreateV6" class="card p-5 space-y-2"><div class="eyebrow">People</div><h3 class="font-extrabold mt-1">Add student</h3><input id="svName" class="field" required placeholder="Full name"><input id="svEmail" type="email" class="field" required placeholder="Email"><input id="svPassword" minlength="8" class="field" required placeholder="Initial password"><div class="grid grid-cols-2 gap-2"><input id="svClass" class="field" placeholder="IT-A"><input id="svRoll" class="field" placeholder="Roll number"></div><input id="svSemester" class="field" placeholder="Semester"><button class="primary w-full">Create student</button></form><div class="card p-5"><div class="flex flex-col sm:flex-row justify-between gap-3"><div><div class="eyebrow">People</div><h3 class="font-extrabold mt-1">Students</h3><p class="text-xs text-slate-500 mt-1">Active student accounts and academic identity.</p></div><input id="studentFilter" class="field sm:w-64" placeholder="Search students"></div><div class="overflow-auto mt-4"><table class="w-full text-sm"><thead><tr class="text-left text-[10px] uppercase text-slate-400 border-b"><th class="py-2">Name</th><th>Roll</th><th>Class</th><th>Semester</th><th>Email</th><th></th></tr></thead><tbody id="studentRows">'+students.map(x=>'<tr class="border-b last:border-0" data-student-row><td class="py-3 font-semibold">'+E(x.name)+'</td><td>'+E(x.roll_no||'—')+'</td><td>'+E(x.class_code||'—')+'</td><td>'+E(x.semester||'—')+'</td><td>'+E(x.email)+'</td><td><button class="text-xs text-blue-600" data-student-edit="'+x.id+'">Edit</button></td></tr>').join('')+'</tbody></table></div></div>';
      ...
      $('studentCreateV6').onsubmit=async e=>{e.preventDefault();try{await api('/api/users',{method:'POST',body:JSON.stringify({name:$('svName').value,email:$('svEmail').value,password:$('svPassword').value,role:'STUDENT',class_code:$('svClass').value,roll_no:$('svRoll').value,semester:$('svSemester').value})});msg('Student created');adminLoad('students')}catch(err){msg(err.message)}};p.querySelectorAll('[data-student-edit]').forEach(b=>b.onclick=()=>adminEditUser(b.dataset.studentEdit));
      return;
    }
    ...
    if(section==='security & audit'){
      const q=await api('/api/admin/audit-logs?limit=100');
      $('adminContent').innerHTML='<div class="card p-5"><div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3"><div><div class="eyebrow">Operations</div><h3 class="font-extrabold mt-1">Security & audit</h3><p class="text-xs text-slate-500 mt-1">Recent administrative and authentication activity.</p></div><div id="auditTotal" class="text-xs text-slate-500"></div></div><div class="grid md:grid-cols-3 gap-2 mt-4"><input id="auditSearch" class="field" placeholder="Search action, user or entity"><select id="auditAction" class="field"><option value="">All actions</option><option>CREATE</option><option>UPDATE</option><option>DELETE</option><option>PASSWORD_CHANGE</option><option>ADMIN_PASSWORD_RESET</option><option>REVOKE_SESSION</option><option>REVOKE_OTHER_SESSIONS</option><option>ADMIN_REVOKE_SESSIONS</option><option>UPLOAD</option></select><select id="auditEntity" class="field"><option value="">All entities</option><option>user</option><option>profile</option><option>auth_session</option><option>notice</option><option>class</option><option>attendance</option><option>marks</option><option>file</option></select></div><div id="auditRows" class="mt-4 space-y-2"></div></div>';
      const loadAudit=async()=>{try{
        const qs=new URLSearchParams({limit:'100'});
        if($('auditSearch').value.trim())qs.set('search',$('auditSearch').value.trim());
        if($('auditAction').value)qs.set('action',$('auditAction').value);
        if($('auditEntity').value)qs.set('entity',$('auditEntity').value);
        const d=await api('/api/admin/audit-logs?'+qs.toString());
        $('auditTotal').textContent=E(d.total)+' total records';
        $('auditRows').innerHTML=d.items.map(x=>'<div class="border rounded-xl p-3 flex flex-col md:flex-row md:items-center md:justify-between gap-2"><div><div class="flex items-center gap-2"><span class="px-2 py-1 rounded-lg bg-blue-50 text-blue-700 text-[10px] font-bold">'+E(x.action)+'</span><span class="text-xs font-semibold">'+E(x.entity)+(x.entity_id?' #'+E(x.entity_id):'')+'</span></div><div class="text-xs text-slate-500 mt-1">'+E(x.user_name||'System')+' • '+E(x.user_email||'')+'</div></div><div class="text-[10px] text-slate-400">'+new Date(x.created_at).toLocaleString()+'</div></div>').join('')||'<p class="text-sm text-slate-500">No audit records match the filters.</p>';
      }catch(err){$('auditRows').innerHTML='<p class="text-sm text-red-600">'+E(err.message)+'</p>'}};
      $('auditSearch').oninput=()=>{clearTimeout(window.auditTimer);window.auditTimer=setTimeout(loadAudit,250)};
      $('auditAction').onchange=loadAudit;$('auditEntity').onchange=loadAudit;loadAudit();return;
    }
    ...
    if(section==='attendance'||section==='marks'){
      ...
      $('adminContent').innerHTML='<div class="grid xl:grid-cols-[360px_1fr] gap-4"><form id="recordCreateV6" class="card p-5 space-y-2"><div class="eyebrow">Academic records</div><h3 class="font-extrabold mt-1">Add '+title.slice(0,-1)+'</h3><input id="rvStudent" type="number" class="field" required placeholder="Student ID"><input id="rvSubject" class="field" required placeholder="Subject"><input id="rvDate" type="date" class="field" required>'+ (section==='attendance'?'<select id="rvStatus" class="field"><option value="true">Present</option><option value="false">Absent</option></select><input id="rvRemarks" class="field" placeholder="Remarks">':'<input id="rvScore" type="number" step="0.01" class="field" required placeholder="Marks"><input id="rvMax" type="number" step="0.01" class="field" value="100" required placeholder="Max marks"><input id="rvExam" class="field" placeholder="Exam type">')+'<button class="primary w-full">Save record</button></form><div class="card p-5"><div class="flex justify-between items-center"><div><div class="eyebrow">Academic records</div><h3 class="font-extrabold mt-1">'+title+'</h3></div><span class="text-xs text-slate-400">'+count+' records</span></div><div class="overflow-auto mt-4"><table class="w-full text-sm"><thead><tr class="text-left text-[10px] uppercase text-slate-400 border-b"><th class="py-2">Student</th><th>Subject</th><th>Date</th><th>'+ (section==='attendance'?'Status':'Score')+'</th><th></th></tr></thead><tbody id="recordRows"></tbody></table></div></div>';
      $('recordCreateV6').onsubmit=async e=>{e.preventDefault();try{if(section==='attendance')await api('/api/attendance',{method:'POST',body:JSON.stringify({student_id:$('rvStudent').value,subject:$('rvSubject').value,date:$('rvDate').value,present:$('rvStatus').value==='true',remarks:$('rvRemarks').value})});else await api('/api/marks',{method:'POST',body:JSON.stringify({student_id:$('rvStudent').value,subject:$('rvSubject').value,marks:$('rvScore').value,max_marks:$('rvMax').value,exam_type:$('rvExam').value||'Internal',exam_date:$('rvDate').value})});msg('Record saved');adminLoad(section)}catch(err){msg(err.message)}};$('recordRows').innerHTML=rows.map(function(x){
