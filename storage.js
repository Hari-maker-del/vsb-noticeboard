...
async function getUploadUrl(key){
  ...
  const root=path.resolve(localDir),safe=path.resolve(root,key);
  if(!safe.startsWith(root+path.sep))throw new Error('Invalid upload key');
  return '/uploads/'+encodeURIComponent(path.basename(safe));
}
async function saveUpload(file){
  ...
  const filename=crypto.randomUUID()+ext,full=path.join(localDir,filename);
